/*Một thư viện cần quản lý các tài liệu bao gồm Sách, Tạp chí, Báo.
Mỗi tài liệu gồm có các thuộc tính sau : Mã tài liệu(Mã tài liệu là duy nhất), Tên nhà xuất bản, số bản phát hành.

Các loại sách cần quản lý thêm các thuộc tính : tên tác giả, số trang.

Các tạp chí cần quản lý thêm : Số phát hành, tháng phát hành.

Các báo cần quản lý thêm : Ngày phát hành.

Yêu cầu 1 : Xây dựng các lớp để quản lý tài liệu cho thư viện một cách hiệu quả.

Yêu cầu 2 : Xây dựng lớp QuanLySach có các chức năng sau

Thêm mới tài liêu : Sách, tạp chí, báo.
Xoá tài liệu theo mã tài liệu.
Hiện thị thông tin về tài liệu.
Tìm kiếm tài liệu theo loại : Sách, tạp chí, báo.
Thoát khỏi chương trình.*/
#include"Source.h"
int main()
{
	QuanLySach List;
	int nChon{ 0 };
	do
	{
		//system("cls");
		std::cout << "||================ Documents managerment systems ================||" << std::endl;
		std::cout << "||                      1. Them tai lieu                         ||" << std::endl;
		std::cout << "||                      2. Xoa tai lieu                          ||" << std::endl;
		std::cout << "||                      3. Hien Thi tai lieu                     ||" << std::endl;
		std::cout << "||                      4. Tim kiem tai lieu                     ||" << std::endl;
		std::cout << "||                      5. Thoat                                 ||" << std::endl;
		std::cout << "||===============================================================||" << std::endl;
		std::cin >> nChon;
		switch (nChon)
		{
		case 1: 
		{
			system("cls");
			List.ThemMoiTaiLieu();
			List.KiemTraTrungLap();
			break;
		}
		case 2:
		{
			system("cls");
			List.XoaTaiLieu();
			break;
		}
		case 3:
		{
			system("cls");
			List.HienThiThongTinTaiLieu();
			break;
		}
		case 4:
		{
			system("cls");
			List.TimKiemTaiLieu();
			break;
		}
		}
		system("cls");

	} while (nChon != 5);
	return 0;
}
